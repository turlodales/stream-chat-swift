...
//        XCTAssertEqual(env.channelUpdater?.createNewMessage_arguments, arguments)
    ...
    func test_loadNextReplies_propagatesError() {
        ...
        var completionError: Error?
        controller.loadNextReplies { [callbackQueueID] in
            AssertTestQueue(withId: callbackQueueID)
            completionError = $0
        }
    ...
    func test_loadPreviousReplies_failsOnEmptyReplies() throws {
        // Simulate `loadPreviousReplies` call and catch the completion error.
        let completionError = try await {
            controller.loadPreviousReplies(completion: $0)
        }
    ...
    func test_loadPreviousReplies_propagatesError() {
        ...
        var completionError: Error?
        controller.loadPreviousReplies(before: .unique) { [callbackQueueID] in
            AssertTestQueue(withId: callbackQueueID)
            completionError = $0
        }
    ...
    func test_loadPreviousReplies_callsMessageUpdater_withCorrectValues() {
        // Simulate `loadPreviousReplies` call
        let beforeMessageId: MessageId = .unique
        controller.loadPreviousReplies(before: beforeMessageId)
        
        // Assert message updater is called with correct values
        XCTAssertEqual(env.messageUpdater.loadReplies_cid, controller.cid)
        XCTAssertEqual(env.messageUpdater.loadReplies_messageId, messageId)
        XCTAssertEqual(env.messageUpdater.loadReplies_pagination, .init(pageSize: 25, parameter: .greaterThan(beforeMessageId)))
    }
}
